...
Commit messages, PR descriptions, and everyday branch utilities.

## Model-invoked

Rich trigger phrasing so the agent reaches for them (also user-reachable). Safe to auto-trigger — they generate text or read state, they don't mutate anything.

- **[commit](./commit/SKILL.md)** — Conventional (Angular) commit messages generated from the real diff (shows the message before committing).
...
Reachable only when you type them (`disable-model-invocation: true`; Codex: `policy.allow_implicit_invocation: false`). These change git state or a pull request, so you invoke them deliberately.
...
- **[changelog](./changelog/SKILL.md)** — Generate the PR description from the diff and write it into the open PR (or offer to open one with `pr`).
